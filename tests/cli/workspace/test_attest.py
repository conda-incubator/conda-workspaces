...
def workspace_verification(
    ...
    signer = SignerIdentity(_SIGNER_POLICY.identity, _SIGNER_POLICY.issuer)
...
def test_execute_verify_reports_authenticated_evidence(
    ...
    signer_args = (
        {
            "cert_identity": _SIGNER_POLICY.identity,
            "cert_oidc_issuer": _SIGNER_POLICY.issuer,
        }
        if authorize
        else {}
    )
    ...
    if json_output:
        assert json.loads(output) == {
            ...
            "signer": {
                "identity": _SIGNER_POLICY.identity,
                "issuer": _SIGNER_POLICY.issuer,
                "timestamps": [
        ...
        assert output == (
            f"Verified workspace attestation from {_SIGNER_POLICY.identity} "
            f"({authorization})\n"
        )
...
@pytest.mark.parametrize(
    ("identity", "issuer"),
    [
        (_SIGNER_POLICY.identity, None),
        (None, _SIGNER_POLICY.issuer),
    ],
