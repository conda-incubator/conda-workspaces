...
from ...paths import atomic_write_text, validate_file_output
...
def execute_update(args: argparse.Namespace, *, console: Console | None = None) -> int:
    ...
    document = tomlkit.loads(original_text)
    ...
    current = parser.parse_data(document.unwrap(), manifest_path)
    ...
    if baseline_path.is_file():
        candidate = load_lockfile_data(baseline_path.read_bytes())
    ...
    config = parser.parse_data(document.unwrap(), manifest_path)
    ...
    def publish_update(lock_content: str) -> None:
        nonlocal publication_started
        if updated_text != original_text:
            atomic_write_text(manifest_path, updated_text)
        publication_started = True
        write_lockfile(ctx, lock_content)
    ...
    try:
        sync_environments(
            ...
            publish_lockfile=publish_update,
            console=console,
        )
